...
from consistent_hashing import ConsistentHashNodeRing
...
# only sends GET to node(s) if user is not in cache
@lru_cache(5)
def get(userHashcode, client_ring):
    ...
    if(bloomfilter.is_member(userHashcode)):
        ...
        allNodesWithUser = client_ring.get_nodes(key)
        responses = []

        # ***** this code sends GET to all nodes that have this user *****
        # for node in allNodesWithUser:
        #     response = node.send(data_bytes)
        #     print(response)
        #     responses.append(response)

        # ***** this code only sends GET to master node, not the slave
        #       nodes that also have a copy of the data ******
        response = allNodesWithUser[0].send(data_bytes)
        print(response)
        responses.append(response)
...
# delete user from all nodes that have this user
...
def process(udp_clients):
    client_ring = ConsistentHashNodeRing(udp_clients)
    hash_codes = set()

    # # PUT all users.
    for u in USERS:
        putResponses = put(u, client_ring)
    ...
    for hc in hash_codes:
        print(hc)
        getResponses = get(hc, client_ring)
    
    # DELETE all users
    for hc in hash_codes:
        print(hc)
        deleteResponses = delete(hc, client_ring)
